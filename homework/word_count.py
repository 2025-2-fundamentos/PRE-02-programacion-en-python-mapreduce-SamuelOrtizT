    ...
    def copy_and_number_raw_files_to_input_folder(raw_dir, input_dir, n=5000):
        for file in glob.glob(f"{raw_dir}/*"):
            ...
            for i in range(1, n + 1):
                raw_filename_with_extension = os.path.basename(file) #no usamos .split("/") dado que el símbolo separador depende del sistema operativo
                raw_filename_without_extension = os.path.splitext(raw_filename_with_extension)[0] #split extension --> "file1" ".txt"
...
def wordcount_mapper(sequence):
    ...
    for _, line in sequence:
        line = line.lower()
        line = line.translate(str.maketrans("", "", string.punctuation)) # make transition
        line = line.replace("\n", "")
        words = line.split()
